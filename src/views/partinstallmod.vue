 ...
 * @LastEditTime: 2020-09-29 08:46:29
 * @LastEditors: gao shuai
 -->
<template>
  <div class="partinstall">
    <div class="gs_title" style="color: white;">创建模板</div>
    <!-- 基本信息 -->
      ...
      <el-collapse-transition>
        <div v-show="collspa.show1" class="gs_listcontainer">
          ...
          <dl class="clearfix" v-if="baseInfo.type===4">
            ...
            <dd>
              <span class="inputbox">
                <el-input size="small" ref="baseGameMing" v-model="baseInfo.gameMing" placeholder="请输入游戏名称"></el-input>
              </span>
              <span class="line_tip">1-16个字，请勿输入特殊符号</span>
            </dd>
          </dl>
          ...
          <dl class="clearfix" v-if="baseInfo.type===3||baseInfo.type===4||baseInfo.type===5">
            ...
            <dd>
              ...
              <span class="line_tip">设置按 游戏账号 或 游戏角色充值，默认按 游戏账号 充值</span>
            </dd>
          </dl>
          <dl class="clearfix">
            <dt>游戏币名称：</dt>
          ...
          <dl class="clearfix" v-if="baseInfo.type===1||baseInfo.type===2">
            ...
            <dd>
              <span class="inputbox">
                <el-input size="small" ref="baseWebCommand" v-model="baseInfo.webCommand" placeholder="请输入浏览器指令"></el-input>
              </span>
                ...
                <div class="tablebox legacy-additional-detail-table" style="width: 100%; max-width: 1000px;">
                  <table style="width: 100%; min-width: 0; table-layout: fixed;">
                    <colgroup>
                      ...
                      <col style="width: 220px;" /><col style="width: 140px;" /><col style="width: 140px;" /><col style="width: 100px;" />
                    </colgroup>
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      <tr v-for="(item,i) in inciteInfo.giveList" :key="'incite'+i">
                        <td>
                          ...
                          <el-input style="display:inline-block;width: 150px;" :ref="'inciteAmount'+i" size="small" :disabled="!inciteInfo.checked" type="number" v-model="item.amount"></el-input> 元
                        </td>
                        <td>
                  ...
                  <table>
                    ...
                    <tbody>
                      <tr v-for="(item,i) in integralInfo.integralList" :key="'integral'+i">
                        <td>
                          <el-input size="small" style="width:90px;" :ref="'integralName'+i" @input="onChangeIntegralName(i)" v-model="item.name"></el-input>
                        </td>
                        <td>
                          <el-input size="small" style="width:250px;" :ref="'integralFile'+i" v-model="item.file"></el-input>
                        </td>
                        <td>
...
export default {
  ...
  methods: {
    ...
    getproductlist() {
      this.$api.partmodule
        ...
        .then((data) => {
          if (data.status === 200) {
            let res = data.data;
            this.chargeInfo.redNPC = res.map((item) => {
              return {
                productId: item.id,
        ...
        this.integralInfo.integralList = [
          {
            ...
            show: 2,
            isShow: false,
            file: '...\\..\\..\\..\\通区充值\\Mir200\\Envir\\QuestDiary\\ChenXu\\充值积分\\元宝消费Save.txt',
            ratio: 1,
            type: 0
          },
          {
            ...
            show: 2,
            isShow: false,
            file: '..\\..\\..\\..\\通区充值\\Mir200\\Envir\\QuestDiary\\ChenXu\\充值积分\\消费积分Save.txt',
            ratio: 1,
            type: 0
          }
        ];// 积分详情;
        ...
        this.integralInfo.integralList = [
          {
            ...
            show: 2,
            isShow: false,
            file: '..\\QuestDiary\\ChenXu\\充值积分\\元宝消费Save.txt',
            ratio: 1,
            type: 0
          },
          {
            ...
            show: 2,
            isShow: false,
            file: '..\\QuestDiary\\ChenXu\\充值积分\\消费积分Save.txt',
            ratio: 1,
            type: 0
          }
        ];// 积分详情;
    ...
    inputCheck() {
      ...
      if (this.baseInfo.type === 1 || this.baseInfo.type === 2) {
        ...
        if (this.baseInfo.gameEngine === '') {
          this.collspa.show1 = true;
          this.$nextTick(() => {
            this.$refs.baseGameEngine.focus();
            this.$messageError('请选择游戏引擎！');
          });
          return true;
        } else if (this.baseInfo.webCommand === '') {
          this.collspa.show1 = true;
          this.$nextTick(() => {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      if (from.name === 'partmodules') {
        vm.routerLink = '/main/partmodules';
      } else if (from.name === 'partinstalls') {
        vm.routerLink = '/main/partinstalls';
      }
    });
  }
};
...
.partinstall .legacy-additional-detail-table {
  ...
  table {
    ...
    th:nth-child(4), td:nth-child(4) { width: 220px !important; }
