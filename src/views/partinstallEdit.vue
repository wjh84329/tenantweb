 ...
 * @LastEditTime: 2020-09-10 10:54:21
    ...
    <div class="bg_fff">
      ...
      <div class="gs_listcontainer">
            ...
            <ul class="areaContainer clearfix">
              ...
              <li class="on" v-if="typeindex === 6">
                <el-tooltip
                  ...
                  placement="bottom"
                >
                  <img src="../assets/images/qiji.png" alt="" />
                </el-tooltip>
              </li>
            </ul>
          </dd>
        ...
        <dl class="clearfix">
          ...
          <dd>
            <span class="inputbox">
              <el-checkbox v-model="partChecked"
                >在指定时间更改分区名称</el-checkbox
              >
            </span>
          </dd>
        </dl>
        ...
        <dl class="clearfix">
          ...
          <dd>
            <span
              class="inputbox clearfix gs_checkbox pdt5"
              style="width:700px;"
            >
              <el-checkbox-group
                class="fl"
                style="max-width:600px;"
                v-model="teamdata.teamlist"
                @change="teamchange"
              >
                <el-checkbox
                  v-for="(team, i) in teamdata.Options"
                  :label="team.id"
                  :key="'team' + i"
                  >{{ team.name }}</el-checkbox
                >
              </el-checkbox-group>
              <el-checkbox
                class="fl mgl20"
                :indeterminate="teamdata.isIndeterminate"
                v-model="teamdata.teamAll"
                @change="teamAllChange"
                >全选</el-checkbox
              >
            </span>
          </dd>
              ...
              <li class="clearfix">
                ...
                <div class="cont gs_textarea1">
                  <el-input
                    ...
                    ref="baseSqlsql"
                    v-model="baseSql.sql"
                    placeholder="如： INSERT INTO `charge` (`id`, `accountname`, `coin`,`state`,`add_time`,`update_time`,`deleted`,`money`,`type`) VALUES (null, '@USER@',@SUM@,0,NOW(),NOW(),0,@MONEY@,NULL);"
                  ></el-input>
                </div>
              </li>
            </ul>
            ...
            <ul class="sqlsetbox">
              ...
              <li class="clearfix">
                ...
                <div class="cont">
                  <el-input
                    size="small"
                    ref="webSetsuccessmark"
                    v-model="webSet.successmark"
                    style="width:325px;"
                    placeholder="请输入内容"
                  ></el-input>
                  <span class="line_tip"
                    >即同步通地址,不能带参数。一般可以不填写</span
                  >
                </div>
              </li>
            </ul>
          ...
          <dd>
            <span class="inputbox">
              <span style="color:red;"
                >如果现在不需要创建脚本，您可以在分区管理中加载脚本</span
              >
            </span>
          </dd>
        </dl>
      ...
      <p class="tc pdb20">
        <el-button type="primary" @click="partinstall" :loading="!installFlag"
          >保存</el-button
        >
      </p>
    </div>
...
export default {
  data() {
    return {
      ...
      gatewayIp: '',
      gatewayPort: '',
      serverIp: '', // 当前分区绑定的 machineCode（加载后用于回填 IP/端口）
      serverDrow: [], // 已注册网关列表
      // serverPort: '', // 通讯端口
      installPath: '', // 安装路径
      installModel: '', // 模板
  ...
  methods: {
    ...
    getareainfo() {
      this.$api.groupmange
        .getAreaInfo({ id: this.$route.query.id })
        .then(data => {
          console.log(data);
          ...
          if (data.data.dbInfo === null) {
            ...
            this.baseSql.type = data.data.dbInfo.type;
            this.baseSql.ip = data.data.dbInfo.ip;
            this.baseSql.port = data.data.dbInfo.port;
            this.baseSql.user = data.data.dbInfo.user;
            this.baseSql.pwd = data.data.dbInfo.pwd;
            this.baseSql.sql = data.data.dbInfo.sql;
            this.baseSql.dbName = data.data.dbInfo.dbName;
          }
          // web
          this.webSet.address = data.data.webUrl; // 接收地址
          this.webSet.successmark = data.data.successMark; // 成功标识
          this.webSet.datatype = data.data.dataFormat; // 数据格式
    ...
    partinstall() {
      if (this.installFlag) {
        ...
        let editMachineCode = this.serverIp;
        if (this.gatewayTypesNeedEndpoint()) {
          const er = this.resolveMachineCodeFromEndpoint();
          if (!er.ok) {
            this.$messageError(er.message);
            this.installFlag = true;
            return;
          }
          editMachineCode = er.machineCode;
        }
        this.$api.partinstall
          .partEdit({
    ...
    //   this.glodEgg = true; // 元宝蛋
    //   this.glodScan = true; // 扫码充值
    //   this.notice = ''; // 分区公告
    //   // sql通用
    //   this.baseSql.type = 0; // 类型
    //   this.baseSql.ip = ''; // ip
